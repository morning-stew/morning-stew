 ...
 *   pnpm exec tsx -r ./src/load-env.cjs src/cli/test-deep-enrich.ts https://your-link.com
 *
 * Run with default test URLs:
 *   pnpm exec tsx -r ./src/load-env.cjs src/cli/test-deep-enrich.ts
 */
...
async function main() {
  ...
  const testUrls = cliUrl
    ? [{ url: cliUrl, label: "CLI-provided URL" }]
    : DEFAULT_TEST_URLS;
